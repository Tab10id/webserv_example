class Response
  ...
  def content_type
    if !file_exists? || file_name.end_with?('.html') || file_name.end_with?('.slim')
      'text/html'
    elsif file_exists? && file_name.end_with?('.css')
      'text/css'
    else
  ...
  def file_content
    return @file_content if instance_variable_defined?(:@file_content)

    if file_name.end_with?('.slim')
      Slim::Template.new(file_path).render
    else
      File.read(file_path)
    end
  end
  ...
  def file_name
    if request.path == '/'
      '/index.html'
    elsif (request.path == '/submit/data') && (request.method == 'POST')
